...
const SpkReusebleJobs = defineAsyncComponent(() =>
  import(
    "@/shared/components/@spk/dashboards/jobs/dashboard/spk-reuseble-jobs.vue"
  ),
);
const SektorAnalytics = defineAsyncComponent(() =>
  import("@/components/dashboards/sektor-analytics.vue"),
);
const InsightCard = defineAsyncComponent(() =>
  import("@/components/dashboard-widgets/InsightCard.vue"),
);
const ActionCenter = defineAsyncComponent(() =>
  import("@/components/dashboard-widgets/ActionCenter.vue"),
);
const ActivityFeed = defineAsyncComponent(() =>
  import("@/components/dashboard-widgets/ActivityFeed.vue"),
);
const QuickActions = defineAsyncComponent(() =>
  import("@/components/dashboard-widgets/QuickActions.vue"),
);
const DrillDownModal = defineAsyncComponent(() =>
  import("@/components/dashboard-widgets/DrillDownModal.vue"),
);
...
const dashboardSkeletonVars = computed(() =>
  isDashboardDarkMode.value
    ? {
        ...
        "--dashboard-skeleton-block-shine":
          "linear-gradient(90deg, rgba(71, 85, 105, 0.34) 0%, rgba(148, 163, 184, 0.28) 50%, rgba(71, 85, 105, 0.34) 100%)",
...
const isDashboardRevealReady = () =>
  !loading.value && dashboardDetailsReady.value && lowerSectionsReady.value;
...
const maybePlayDashboardLoaderExit = () => {
  if (
    showDashboardLoader.value &&
    !dashboardLoaderExiting.value &&
    isDashboardRevealReady()
  ) {
      ...
      if (root) {
        gsap.set(
          root.querySelectorAll(
            ".dashboard-loading-card, .dashboard-loader-stage, .bouncing-loader-ball, .bouncing-loader-shard",
          ),
          {
...
const availableYearOptions = computed(() => {
  ...
  baseCsirts.value.forEach((item) =>
    pushYear(item?.perusahaan?.created_at || item?.created_at),
  );
  baseSdm.value.forEach((item) => pushYear(item?.created_at));
  baseIkasStakeholders.value.forEach((item) =>
    pushYear(item?.updated_at || item?.created_at),
  );
...
function isInCategoryQuarter(dateStr, targetYear, targetQuarter) {
  ...
  return (
    d.getFullYear() === parseInt(targetYear) && q === parseInt(targetQuarter)
  );
}
...
const getSeCompany = (se) => {
  ...
  const companyId =
    ...
    csirt?.perusahaan?.id;
  return companyId
    ? stakeholdersStore.stakeholdersByIdMap[String(companyId)] ||
        se?.perusahaan ||
        csirt?.perusahaan ||
        null
    : null;
...
const localKseList = computed(() => {
  const kategori = String(filterStore.kategoriSe || "")
    .trim()
    .toLowerCase();
...
const localKseData = computed(() => {
  ...
  const activeCategory = String(filterStore.kategoriSe || "")
    .trim()
    .toLowerCase();
  let strategis = 0,
    tinggi = 0,
...
const hasUsableApiKseData = computed(() => {
  ...
  const hasApiNumbers =
    ...
    data.rendah > 0;
  return (
    !!filterStore.summaryData?.endpointStatus?.se?.ok &&
    (hasApiNumbers || datedFirstKseByCompany.value.length === 0)
  );
});
...
const kseStatus = computed(() => {
  ...
  const totalPerusahaan =
    apiStatus.total_perusahaan || datedStakeholders.value.length;
  const sudahFromStatus = apiStatus.sudah_mengisi_kse || 0;
  const hasApiStatusNumbers =
    apiStatus.total_perusahaan > 0 ||
    apiStatus.sudah_mengisi_kse > 0 ||
    apiStatus.belum_mengisi_kse > 0;
...
const resolveSektorNameById = (id) => {
  if (!id) return "";
  const sektor = sektorList.value.find(
...
const resolveSubSektorNameById = (id) => {
  if (!id) return "";
  const subSektor = subSektorList.value.find(
    (item) => String(item.id) === String(id),
  );
  ...
  return counts
    .map((item) => {
      ...
      return {
        ...
        countYear:
          Number(item?.countYear ?? item?.count_year ?? item?.tahun_ini ?? 0) ||
          0,
        countQuarter:
          Number(
            item?.countQuarter ?? item?.count_quarter ?? item?.kuartal_ini ?? 0,
          ) || 0,
      };
    })
    .filter(
...
const globalCsirtStatus = computed(
  () => filterStore.globalSummaryData?.csirt_status || {},
);
const globalIkasStatus = computed(
  () => filterStore.globalSummaryData?.ikas_status || {},
);
const globalKseData = computed(
  () => filterStore.globalSummaryData?.kse_summary || {},
);
...
const getCsirtCompanyId = (csirt) =>
  csirt?.id_perusahaan || csirt?.perusahaan?.id || "";
const getCsirtDate = (csirt) =>
  csirt?.created_at || csirt?.createdAt || csirt?.perusahaan?.created_at || "";
const hasCsirtSdm = (csirt) => {
  const cid = String(csirt?.id || "");
  return (
    !!(cid && csirtStore.sdmByCsirtMap[cid]?.length) ||
    (Array.isArray(csirt?.sdm_csirt) && csirt.sdm_csirt.length > 0)
  );
};
const hasCsirtSe = (csirt) => {
  ...
  return (
    !!(cid && csirtStore.seByCsirtMap[cid]?.length) ||
...
const csirtStatus = computed(() => {
  ...
  const hasApiStatusNumbers =
    apiStatus.total_perusahaan > 0 ||
    apiStatus.sudah_membentuk_csirt > 0 ||
    apiStatus.belum_membentuk_csirt > 0;
  ...
  return {
    ...
    belum_membentuk_csirt: Math.max(
      0,
      totalStakeholders.value - companyIds.size,
    ),
  };
});
...
const csirtCompletionRateAnalytics = computed(() => {
  ...
  return Math.round(
    (status.sudah_membentuk_csirt / status.total_perusahaan) * 100,
  );
});
...
const manrisCompletionRate = computed(() => {
  ...
  if (!status.total_perusahaan) return 0;
  return Math.round(
    (status.sudah_mengisi_manris / status.total_perusahaan) * 100,
  );
});
...
const ikasSummaryData = computed(() => {
  ...
  const filtered =
    range && (range[0] || range[1])
      ? stakeholders.filter((s) =>
          isInDateRange(getStakeholderIkasDate(s), range),
        )
      : stakeholders;
  ...
  filtered.forEach((s) => {
    ...
    [idenTotal, idenCount] = addDomainValue(
      data.identifikasi?.nilai_identifikasi,
      idenTotal,
      idenCount,
    );
    [protTotal, protCount] = addDomainValue(
      data.proteksi?.nilai_proteksi,
      protTotal,
      protCount,
    );
    [detTotal, detCount] = addDomainValue(
      data.deteksi?.nilai_deteksi,
      detTotal,
      detCount,
    );
    [tangTotal, tangCount] = addDomainValue(
...
const buildAnalyticsChartData = (view = "distribution") => {
  if (summaryMode.value === "KSE") {
    ...
    const status = manrisStatus.value;
    const localInProgress = Object.entries(resikoStore.answersMap || {}).filter(
...
const analyticsChartData = computed(() =>
  buildAnalyticsChartData(analyticsView.value),
);

const distributionAnalyticsData = computed(() =>
  buildAnalyticsChartData("distribution"),
);
const completionAnalyticsData = computed(() =>
  buildAnalyticsChartData("completion"),
);
...
const formatLegendItem = (seriesName, opts) => {
  ...
  return `${seriesName}  ${formatChartNumber(value)} (${formatSlicePercent(
    percent,
  )})`;
};
...
const buildAnalyticsTooltip = (data) => ({
  ...
  custom: ({ series, seriesIndex, dataPointIndex, w }) => {
    ...
    const value = Number(
      isAxisSeries
        ? series?.[seriesIndex]?.[itemIndex]
        : series?.[seriesIndex] || 0,
    );
    const allSeries = isAxisSeries
      ? series?.[seriesIndex] || []
      : w?.globals?.series || [];
    const label =
      data.labels?.[itemIndex] || w?.globals?.labels?.[itemIndex] || "Data";
...
const distributionVisualItems = computed(() =>
  buildVisualItems(distributionAnalyticsData.value),
);
const completionVisualItems = computed(() =>
  buildVisualItems(completionAnalyticsData.value),
);
            ...
            labels: {
              ...
              total: {
                ...
                label:
                  view === "completion"
                    ? "Total Status"
                    : summaryMode.value === "KSE"
                    ? "Total KSE"
                    : summaryMode.value === "CSIRT"
                    ? "Total CSIRT"
                    : summaryMode.value === "MANRIS"
                    ? "Total Manris"
                    : "Total IKAS",
                fontSize: "12px",
                fontWeight: 850,
...
const distributionAnalyticsOptions = computed(() =>
  buildAnalyticsOptions(
    distributionAnalyticsData.value,
    activeChartType.value,
    "distribution",
  ),
);
const distributionAnalyticsSeries = computed(() =>
  buildAnalyticsSeries(distributionAnalyticsData.value, activeChartType.value),
);
const completionAnalyticsOptions = computed(() =>
  buildAnalyticsOptions(
    completionAnalyticsData.value,
    completionChartType.value,
    "completion",
  ),
);
const completionAnalyticsSeries = computed(() =>
...
const activeFilledCount = computed(() => {
  if (summaryMode.value === "KSE")
    return kseStatus.value?.sudah_mengisi_kse ?? 0;
  if (summaryMode.value === "CSIRT")
    return csirtStatus.value?.sudah_membentuk_csirt ?? 0;
  if (summaryMode.value === "MANRIS")
    return manrisStatus.value.sudah_mengisi_manris;
  return ikasFilledCount.value;
});

const activeUnfilledCount = computed(() => {
  if (summaryMode.value === "KSE")
    return kseStatus.value?.belum_mengisi_kse ?? 0;
  if (summaryMode.value === "CSIRT")
    return csirtStatus.value?.belum_membentuk_csirt ?? 0;
  if (summaryMode.value === "MANRIS")
    return manrisStatus.value.belum_mengisi_manris;
  return ikasUnfilledCount.value;
});

const activeStatusTotal = computed(() => {
  if (summaryMode.value === "KSE")
    return kseStatus.value?.total_perusahaan ?? 0;
  if (summaryMode.value === "CSIRT")
    return csirtStatus.value?.total_perusahaan ?? 0;
  if (summaryMode.value === "MANRIS")
    return manrisStatus.value.total_perusahaan;
...
const fullSummaryItems = computed(() => {
  ...
  if (summaryMode.value === "CSIRT") {
    ...
    const items = [
      ...
      {
        label: filterStore.quarter
          ? `CSIRT Q${filterStore.quarter}`
          : filterStore.year
          ? `CSIRT ${filterStore.year}`
          : "CSIRT Periode Ini",
  ...
  if (summaryMode.value === "MANRIS") {
    ...
    const trained = respondents.filter((item) =>
      String(item.sertifikat_training || "").trim(),
    ).length;
    const items = [
      {
  ...
  if (katSe) {
    return items.map((item) => ({
      ...
      isMuted: !(
        ...
        item.category === katSe ||
        item.label.includes("SE Q") ||
        item.label.includes("SE 20") ||
        item.label.includes("Periode")
      ),
    }));
  }
...
function isInDateRange(createdAt, rangeStrArray) {
  if (!rangeStrArray || !rangeStrArray[0] || !rangeStrArray[1] || !createdAt)
    return false;
...
function getTrendData(items, dateField = "created_at") {
  ...
  items.forEach((item) => {
    const raw =
      getField(item, dateField) || item["created_at"] || item["updated_at"];
...
const loadDashboardOptions = async () => {
  ...
  dashboardOptionsPromise = (async () => {
    const [sektors, subSektors] = await Promise.all([
      sektorList.value.length
        ? Promise.resolve(sektorList.value)
        : sektorService.getAll(),
      subSektorList.value.length
        ? Promise.resolve(subSektorList.value)
...
const loadDashboardData = async (options = {}) => {
  ...
  if (
    ...
    (dashboardLoadInFlight ||
      now - lastDashboardLoadAt < DASHBOARD_RELOAD_DEBOUNCE_MS)
  ) {
    return;
  }
  ...
  const loadStore = (store, refreshMethod = "refresh") => {
    if (
      options.refresh &&
      store.initialized &&
      typeof store[refreshMethod] === "function"
    )
      return store[refreshMethod]();
...
const baseStakeholders = computed(() => {
  ...
  if (fId || subId) {
    filtered = filtered.filter((s) => {
      ...
      if (effectiveSubId)
        return (
          String(s.sub_sektor?.id || s.id_sub_sektor) === String(effectiveSubId)
        );
      if (fId)
        return String(s.sub_sektor?.id_sektor || s.id_sektor) === String(fId);
      return true;
    });
  }
...
const hasKonversiApiData = computed(
  () => konversiStore.initialized && konversiStore.records.length > 0,
);
...
const countCompleteKonversiStakeholders = (stakeholders) =>
  stakeholders.filter((stakeholder) =>
    isKonversiComplete(getStakeholderKonversiRecord(stakeholder)),
  ).length;
...
const baseCsirts = computed(() => {
  ...
  if (fId || subId) {
    filtered = filtered.filter((c) => {
      const p =
        stakeholdersStore.stakeholdersByIdMap[String(c.id_perusahaan)] ||
        c.perusahaan;
      const effectiveSubId = subId === "ALL" ? "" : subId;
      if (effectiveSubId)
        return (
          String(p?.sub_sektor?.id || p?.id_sub_sektor) ===
          String(effectiveSubId)
        );
      if (fId)
...
const baseSdm = computed(() => {
  ...
  return csirtStore.sdmList.filter((s) => {
    ...
    const p =
      stakeholdersStore.stakeholdersByIdMap[String(csirt.id_perusahaan)] ||
      csirt.perusahaan;
    const effectiveSubId = subId === "ALL" ? "" : subId;
    if (effectiveSubId)
      return (
        String(p?.sub_sektor?.id || p?.id_sub_sektor) === String(effectiveSubId)
      );
    if (fId)
      return String(p?.sub_sektor?.id_sektor || p?.id_sektor) === String(fId);
    return true;
...
const baseSeList = computed(() => {
  ...
  if (fId || subId) {
    filtered = filtered.filter((se) => {
      ...
      const pId =
        csirt?.id_perusahaan || csirt?.perusahaan?.id || se.id_perusahaan;
      const perus = pId
        ? stakeholdersStore.stakeholdersByIdMap[String(pId)]
        : null;
      ...
      if (effectiveSubId)
        return (
          String(perus.sub_sektor?.id || perus.id_sub_sektor) ===
          String(effectiveSubId)
        );
      if (fId)
        return (
          String(perus.sub_sektor?.id_sektor || perus.id_sektor) === String(fId)
        );
      return true;
    });
  }
...
const totalCsirt = computed(() => {
  ...
  return baseStakeholders.value.filter(
    (s) => !!csirtStore.csirtByPerusahaanMap[String(s.id)],
  ).length;
});
...
const stakeholdersWithCsirt = computed(() => {
  return baseStakeholders.value.filter((s) => csirtStore.hasCompleteCsirt(s.id))
    .length;
});
...
const csirtCompletionRate = computed(() => {
  ...
  return Math.round(
    (stakeholdersWithCsirt.value / totalStakeholders.value) * 100,
  );
});
...
const activeSubSektorSummary = computed(() => {
  ...
  const target = counts.find(
    (s) => String(s.sektor_id || s.id) === String(subId),
  );
  ...
  const kseCount = baseStakeholders.value.filter((s) => {
    ...
    const se = csirtStore.seList.find(
      (se) => String(se.id_perusahaan) === String(s.id),
    );
  ...
  baseStakeholders.value.forEach((s) => {
    ...
    const hasCsirtRecord = csirtStore.csirts.some(
      (c) =>
        String(c.id_perusahaan) === String(s.id) ||
        String(c.perusahaan?.id) === String(s.id),
    );
    ...
    const hasIkas =
      ikasData &&
      ((ikasData.total_rata_rata &&
        ikasData.total_rata_rata !== "NA" &&
  ...
  return {
    nama_sektor:
      subSektorList.value.find((s) => String(s.id) === String(subId))
        ?.nama_sub_sektor || "Detail Stakeholder",
    total: totalBase,
    countYear: baseStakeholders.value.filter((s) =>
      isInDateRange(s.created_at, [
...
const datedCsirts = computed(() => {
  const range = filterStore.dateRange;
  const completeCsirts = baseStakeholders.value.filter((s) =>
    csirtStore.hasCompleteCsirt(s.id),
  );
...
const datedIkasStakeholders = computed(() => {
  ...
  return baseIkasStakeholders.value.filter((s) =>
    isInDateRange(getStakeholderIkasDate(s), range),
  );
});
const datedSeList = computed(() => {
  ...
  return baseSeList.value.filter((se) => {
    ...
    const csirt = csirtStore.csirts.find(
      (c) => String(c.id) === String(se.id_csirt),
    );
    if (
      csirt?.perusahaan?.created_at &&
      isInDateRange(csirt.perusahaan.created_at, range)
    )
      return true;
    return false;
  });
...
const operationalCardsReady = computed(
  () =>
    !loading.value &&
    (!!filterStore.summaryData || dashboardDetailsReady.value),
);
...
const sektorCards = computed(() => {
      ...
      .map((ss, idx) => {
        ...
        return {
          ...
          cardClass: `dashboard-main-card overflow-hidden ${color} ${
            isActive ? "sektor-card-active" : ""
          } ${isMuted ? "sektor-card-muted" : ""}`,
          priceColor: color,
  ...
  const sorted = [...enrichedSektors.value].sort(
    (a, b) => b.stakeholderCount - a.stakeholderCount,
  );

  let topSektors = sorted.slice(0, 6);

  if (activeSektorId) {
    const hasActive = topSektors.some(
      (s) => String(s.id) === String(activeSektorId),
    );
    if (!hasActive) {
      const activeSektor = sorted.find(
        (s) => String(s.id) === String(activeSektorId),
      );
      if (activeSektor) {
        topSektors.pop();
...
const operationalCards = computed(() => {
  ...
  const csirtLocalReady =
    csirtStore.initialized && stakeholdersStore.initialized;
  ...
  const fixedCsirtCount =
    globalCsirtStatus.value.sudah_membentuk_csirt ||
    apiCsirtStatus.value.sudah_membentuk_csirt ||
    apiCsirtData.value.total_csirt ||
    0;
  const fixedIkasCount =
    globalIkasStatus.value.sudah_mengisi_ikas ||
    apiIkasStatus.value.sudah_mengisi_ikas ||
    apiIkasData.value.total_ikas ||
    0;
  const fixedSeCount =
    globalKseData.value.total_se || apiKseData.value.total_se || 0;

  // Filtered counts for the bottom small numbers (follows global filters)
  const displayFilteredCsirt = csirtLocalReady
    ? filteredCsirt.value
    : apiCsirtStatus.value.sudah_membentuk_csirt ||
      apiCsirtData.value.total_csirt ||
      0;
  const displayFilteredIkas = ikasLocalReady
    ? filteredIkasCount.value
    : apiIkasStatus.value.sudah_mengisi_ikas ||
      apiIkasData.value.total_ikas ||
      0;
  const displayFilteredSe = csirtLocalReady
    ? filteredSe.value
    : apiKseData.value.total_se || 0;
  ...
  const csirtTrend = useDetailedTrends
    ? getTrendData(baseCsirts.value, "perusahaan.created_at")
    : fallbackTrendData(displayFilteredCsirt);
  const ikasTrend = useDetailedTrends
    ? getTrendData(baseIkasStakeholders.value, getStakeholderIkasDate)
    : fallbackTrendData(displayFilteredIkas);
  const seTrend = useDetailedTrends
    ? getTrendData(firstKseByCompany.value)
    : fallbackTrendData(displayFilteredSe);
  const stakeholderTrend = useDetailedTrends
    ? getTrendData(baseStakeholders.value)
    : fallbackTrendData(displayFilteredStakeholder);
  ...
  return [
    ...
    },
    {
      ...
      ValueClass1: "fs-12 lh-base",
      svgIcon:
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" fill="none"/><path d="M224,200h-16V64a8,8,0,0,0-8-8H152a8,8,0,0,0-8,8v136H96v-80a8,8,0,0,0-8-8H40a8,8,0,0,0-8,8v80H16a8,8,0,0,0,0,16H224a8,8,0,0,0,0-16ZM160,72h32v128H160ZM48,128H80v72H48Z"/></svg>',
      id: "chart-ikas",
      type: "area",
      height: "50",
      width: "100",
    ...
    },
    {
      ...
      ValueClass1: "fs-12 lh-base",
      svgIcon:
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" fill="none"/><path d="M208,32H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM96,120a32,32,0,1,1,32,32A32,32,0,0,1,96,120Zm-2.15,72a64,64,0,0,1,68.31,0H93.85ZM208,208H178.23a79.42,79.42,0,0,0-36.59-27.71,48,48,0,1,0-27.28,0A79.42,79.42,0,0,0,77.77,208H48V48H208Z"/></svg>',
...
function getStakeholderSektorName(s) {
  ...
  if (subId) {
    const ss = subSektorList.value.find(
  ...
  if (sektorId) {
    const parent = sektorList.value.find(
      (item) => String(item.id) === String(sektorId),
    );
    if (parent) return getSektorName(parent);
  }
...
function getCsirtSektorName(c) {
  if (!c) return "-";
  // Try nested paths
  if (c.perusahaan?.sub_sektor?.nama_sektor)
    return c.perusahaan.sub_sektor.nama_sektor;
  if (c.perusahaan?.sub_sektor?.sektor?.nama_sektor)
    return c.perusahaan.sub_sektor.sektor.nama_sektor;
...
function getCsirtSubSektorName(c) {
  if (!c) return "-";
  // Try nested paths
  if (c.perusahaan?.sub_sektor?.nama_sub_sektor)
    return c.perusahaan.sub_sektor.nama_sub_sektor;
...
async function handleDrillDown(context) {
  if (context?.type === "Data Lengkap") {
    await (konversiStore.initialized
      ? konversiStore.refresh()
      : konversiStore.initialize());
  }
  ...
  if (context.type === "stakeholders" || context.type === "Stakeholders") {
    ...
    context.type === "csirt" ||
    context.type.includes("CSIRT") ||
    context.type.includes("Membentuk")
  ) {
    drillDownColumns.value = [
      "nama_perusahaan",
      "sektor",
    ...
    drillDownItems.value = csirtList.map((c) => {
      ...
      return {
        nama_perusahaan:
          ...
          c.perusahaan?.nama_perusahaan ||
          c.nama_csirt ||
          "-",
        sektor: stakeholder
          ? getStakeholderSektorName(stakeholder)
          : getCsirtSektorName(c),
        sub_sektor: stakeholder
          ? getStakeholderSubSektorName(stakeholder)
          : getCsirtSubSektorName(c),
        csirt_nama: c.nama_csirt || "-",
        csirt_status: complete ? "Lengkap" : "Belum Lengkap",
        created_at: getCsirtDate(c)
          ? new Date(getCsirtDate(c)).toLocaleDateString("id-ID")
          : "-",
        slug: stakeholder?.slug || c.perusahaan?.slug || c.id,
      };
    });
  } else if (
    context.type === "Sistem Elektronik" ||
    context.type.includes("SE") ||
    context.type.includes("Total SE")
  ) {
    ...
    if (type.includes("strategis"))
      seList = seList.filter(
        (se) => (se.kategori_se || "").toLowerCase().trim() === "strategis",
      );
    else if (type.includes("tinggi"))
      seList = seList.filter(
        (se) => (se.kategori_se || "").toLowerCase().trim() === "tinggi",
      );
    else if (type.includes("rendah"))
      seList = seList.filter(
        (se) => (se.kategori_se || "").toLowerCase().trim() === "rendah",
      );
      ...
      const activeCategory = String(filterStore.kategoriSe || "")
        ...
        .trim();
      seList = seList.filter(
        (se) => (se.kategori_se || "").toLowerCase().trim() === activeCategory,
      );
    }
    ...
    drillDownItems.value = seList.map((se) => {
      ...
      const subId =
        se.id_sub_sektor ||
        stakeholder?.id_sub_sektor ||
        stakeholder?.sub_sektor?.id;
      ...
      return {
        ...
        sub_sektor:
          seSubSektor !== "-"
            ...
            ? getStakeholderSubSektorName(stakeholder)
            : csirt
            ? getCsirtSubSektorName(csirt)
            : "-",
        nama_se: se.nama_se || se.nama || "-",
        kategori_se: se.kategori_se || "-",
        slug:
          stakeholder?.slug || se.perusahaan?.slug || csirt?.perusahaan?.slug,
      };
    });
    ...
    drillDownItems.value = filteredSt.map((s) => {
      ...
      return {
        ...
        sektor: getStakeholderSektorName(s),
        sub_sektor: getStakeholderSubSektorName(s),
        ikas_score: ikasData?.total_rata_rata
          ? Number(ikasData.total_rata_rata).toFixed(2)
          : "-",
    ...
    drillDownItems.value = all
      .filter((s) => csirtStore.hasCompleteCsirt(s.id))
      .map((s) => {
        const csirtData = csirtStore.csirts.find(
          (c) =>
            String(c.id_perusahaan) === String(s.id) ||
            String(c.perusahaan?.id) === String(s.id),
        );
    ...
    drillDownItems.value = all
      ...
      .filter(({ konversi }) => isKonversiComplete(konversi))
      .map(({ stakeholder: s, konversi, progress }) => ({
        nama_perusahaan:
          konversi?.nama_perusahaan || s.nama_perusahaan || s.nama || "-",
        sektor: getStakeholderSektorName(s),
        sub_sektor: getStakeholderSubSektorName(s),
    ...
    csirtStore.sdmList.forEach((s) => {
      ...
      if (csirtToSt.has(csirtId))
        sdmToSt.set(String(s.id), csirtToSt.get(csirtId));
    });
    const seToSt = new Map();
    csirtStore.seList.forEach((s) => {
      const csirtId = String(s.id_csirt || s.csirt?.id);
      if (csirtToSt.has(csirtId))
        seToSt.set(String(s.id), csirtToSt.get(csirtId));
    });
    ...
    for (const event of notifStore.events) {
      ...
      else if (event.entity === "ikas" || event.entity === "unknown")
        stId = ikasToSt.get(eId) || eId;
      ...
      if (stId && !stLatestUpdate.has(stId)) {
        ...
        stLatestAction.set(
          stId,
          `${userName} ${verbMap[event.type] || "Update"} Data ${
            entityMap[event.entity] || event.entity
          }`,
        );
      }
    }
...
function handleDrillDownNavigate(item) {
  ...
  if (item && item.slug) {
    ...
    if (
      searchTitle.includes("kse") ||
      searchTitle.includes("sistem elektronik") ||
      searchTitle.includes("se ")
    ) {
      router.push({
        path: "/kse",
        query: {
...
const monitoringSektorColorRules = [
  ...
  {
    keywords: ["logam", "mesin", "alat transportasi", "elektronika", "ilmate"],
    color: "#dc2626",
  },
  { keywords: ["kawasan industri"], color: "#f17432" },
];

const normalizeMonitoringLabel = (value) =>
  String(value || "")
    .trim()
    .toLowerCase();
...
const findSektorIndexByName = (label) =>
  sektorList.value.findIndex(
    (item) =>
      normalizeMonitoringLabel(getSektorName(item)) ===
      normalizeMonitoringLabel(label),
  );

const findSubSektorByName = (label) =>
  subSektorList.value.find(
    (item) =>
      normalizeMonitoringLabel(getSubSektorName(item)) ===
      normalizeMonitoringLabel(label),
  );

const getMonitoringSektorColor = (label) => {
  const sektorIndex = findSektorIndexByName(label);
  return getMonitoringSektorBaseColor(
    label,
    sektorIndex >= 0 ? sektorIndex : 0,
  );
};
...
const getMonitoringSubSektorColor = (label, index = 0) => {
  ...
  if (subSektor) {
    ...
    const parentIndex = sektorList.value.findIndex(
...
const resolveMonitoringSubSektorId = (stakeholder) => {
  ...
  if (stakeholder?.sub_sektor?.nama_sub_sektor) {
    const matched = subSektorList.value.find(
      (item) =>
        getSubSektorName(item) === stakeholder.sub_sektor.nama_sub_sektor,
    );
    if (matched) return String(matched.id);
  }
  if (stakeholder?.sektor) {
    const matched = subSektorList.value.find(
      (item) => getSubSektorName(item) === stakeholder.sektor,
    );
    if (matched) return String(matched.id);
  }
  return null;
...
const monitoringChartSektors = computed(() => {
  ...
  let data = sektorList.value
    .map((sektor) => {
      ...
      const subSektors = children
        .map((subSektor) => {
          const stakeholderCount = datedStakeholders.value.filter(
            (stakeholder) => {
        ...
        .filter(
          (subSektor) =>
            subSektor.stakeholderCount > 0 ||
            shouldKeepEmptySubSektor(getSektorName(sektor)),
        );
      ...
      return {
        ...
        subSektors,
        stakeholderCount: subSektors.reduce(
          (total, subSektor) => total + subSektor.stakeholderCount,
          0,
        ),
      };
    })
  ...
  if (filterStore.sektorId) {
    data = data.filter(
      (sektor) => String(sektor.id) === String(filterStore.sektorId),
    );
  }
  ...
  if (filterStore.subSektorId && filterStore.subSektorId !== "ALL") {
    data = data
      .filter((sektor) =>
        sektor.subSektors.some(
          (subSektor) =>
            String(subSektor.id) === String(filterStore.subSektorId),
        ),
      )
      .map((sektor) => {
        const subSektors = sektor.subSektors.filter(
          (subSektor) =>
            String(subSektor.id) === String(filterStore.subSektorId),
        );
        return {
...
const monitoringKpiCards = computed(() => {
  ...
  const csirtFormed =
    csirtStatusData.sudah_membentuk_csirt || csirt.total_csirt;
  ...
  return [
    {
      ...
      sub: `${formatChartNumber(lengkapCount)} data lengkap, ${
        totalSektors.value
      } sektor, ${totalSubSektors.value} sub-sektor`,
      icon: "ri-building-4-line",
      color: "#2563eb",
      rate: lengkapRate,
    },
    ...
    },
    {
      ...
      total: risk.total,
      sub: `${risk.rate}% selesai, ${formatChartNumber(
        risk.inProgress,
      )} proses`,
...
const monitoringDataCompleteRate = computed(() =>
  monitoringPercentOf(
    monitoringDataCompleteCount.value,
    totalStakeholders.value,
  ),
);
...
const monitoringHeroHighlights = computed(() => [
  {
    ...
    value: formatChartNumber(
      monitoringSektorItems.value.length || totalSektors.value,
    ),
...
const monitoringCsirtData = computed(() => ({
  labels: ["Lengkap", "Belum Lengkap", "Punya SDM", "Punya SE"],
  series: [
    csirtData.value.lengkap,
    csirtData.value.belum_lengkap,
    csirtData.value.punya_sdm,
    csirtData.value.punya_se,
  ],
  colors: ["#26bf94", "#f5b849", "#0ea5e9", "#3b82f6"],
}));
...
const monitoringStatusItems = computed(() => [
  {
    label: "KSE sudah",
    value: kseStatus.value.sudah_mengisi_kse,
    color: "#26bf94",
  },
  {
    label: "KSE belum",
    value: kseStatus.value.belum_mengisi_kse,
    color: "#e6533c",
  },
  ...
  {
    label: "CSIRT sudah",
    value: csirtStatus.value.sudah_membentuk_csirt,
    color: "#0891b2",
  },
  {
    label: "CSIRT belum",
    value: csirtStatus.value.belum_membentuk_csirt,
    color: "#64748b",
  },
]);
...
const monitoringIkasDomainItems = computed(() => [
  {
    key: "identifikasi",
    label: "Identifikasi",
    value: Number(ikasSummaryData.value.averages.identifikasi || 0),
    color: "#2563eb",
  ...
  },
  {
    key: "proteksi",
    label: "Proteksi",
    value: Number(ikasSummaryData.value.averages.proteksi || 0),
    color: "#16a34a",
    icon: "ri-shield-check-line",
  },
  {
    key: "deteksi",
    label: "Deteksi",
    value: Number(ikasSummaryData.value.averages.deteksi || 0),
    color: "#d97706",
    icon: "ri-radar-line",
  },
  {
    ...
    value: Number(ikasSummaryData.value.averages.tanggulih || 0),
    color: "#0891b2",
...
const buildDistributionBarOptions = (items) => {
  ...
  return {
    ...
    dataLabels: {
      enabled: true,
      formatter: (value) =>
        value ? `${formatChartNumber(value)} stakeholder` : "",
      textAnchor: "middle",
      offsetX: 0,
      style: {
        colors: ["#ffffff"],
...
const monitoringSektorChartOptions = computed(() =>
  buildDistributionBarOptions(monitoringSektorItems.value),
);
const monitoringSektorChartSeries = computed(() => [
  {
    name: "Stakeholder",
    data: monitoringSektorItems.value.map((item) => item.value),
  },
]);
const monitoringSektorChartHeight = computed(() =>
...
const monitoringIkasChartSeries = computed(() => [
  { name: "IKAS", data: monitoringIkasItems.value.map((item) => item.value) },
]);
const monitoringStatusChartOptions = computed(() =>
  buildMonitoringBarOptions(monitoringStatusItems.value, true),
);
const monitoringStatusChartSeries = computed(() => [
  {
    name: "Status",
    data: monitoringStatusItems.value.map((item) => item.value),
  },
]);
const monitoringKseChartOptions = computed(() =>
  buildMonitoringDonutOptions(monitoringKseData.value, "Total KSE"),
);
const monitoringKseChartSeries = computed(() => monitoringKseData.value.series);
const monitoringCsirtChartOptions = computed(() =>
  buildMonitoringDonutOptions(monitoringCsirtData.value, "CSIRT"),
);
const monitoringCsirtChartSeries = computed(
  () => monitoringCsirtData.value.series,
);

// IKAS Donut Chart (Level Distribution)
const monitoringIkasDonutOptions = computed(() =>
  buildMonitoringDonutOptions(monitoringIkasLevelDonutData.value, "Total IKAS"),
);
const monitoringIkasDonutSeries = computed(
  () => monitoringIkasLevelDonutData.value.series,
);

// IKAS Pie Chart (Domain Averages)
const monitoringIkasPieOptions = computed(() =>
  buildMonitoringPieOptions(monitoringIkasDomainPieData.value),
);
const monitoringIkasPieSeries = computed(
  () => monitoringIkasDomainPieData.value.series,
);
</script>
...
<template>
  <!-- HEADER -->
  <div
    class="d-flex align-items-center justify-content-between mb-2 page-header-breadcrumb flex-wrap gap-2 dashboard-page-header"
  >
    <div class="dashboard-page-heading">
      ...
      <div class="dashboard-header-meta">
        ...
        <span
          ...
          >{{ formatChartNumber(totalStakeholders) }} stakeholder</span
        >
      </div>
    </div>

    <div
      class="dashboard-header-actions d-flex align-items-center flex-wrap gap-2"
    >
      <!-- Datepicker -->
      <div class="dashboard-datepicker-wrapper">
        <Datepicker
          placeholder="Ketik tgl (DD/MM/YYYY) atau Pilih Rentang"
          autoApply
          v-model="date"
          range
          ...
          <template #left-sidebar>
            <div class="dp-custom-range">
              ...
              <div class="dp-sidebar-section">
                ...
                <button
                  class="dp-sidebar-btn dp-btn-solid"
              ...
              <div class="dp-sidebar-section">
                ...
                <p class="dp-sidebar-desc">
                  Klik tahun untuk otomatis ambil 1 Jan - 31 Des.
                </p>
                <select
              ...
              <div class="dp-sidebar-section">
                ...
                <input
                  ...
                  class="dp-date-input"
                  @keydown.enter="applySingleDate"
                />
                <button
                  class="dp-sidebar-btn dp-btn-outline"
      ...
      <!-- Quick Actions -->
      <QuickActions @refresh-data="handleRefreshData" />
      <div>
        <button
          v-if="!showMetabase"
          class="btn dashboard-alt-btn d-flex align-items-center gap-2"
          @click="toggleMetabase"
        >
  ...
  <div
    ...
    :class="{ 'is-dark': isDashboardDarkMode }"
  >
    ...
    >
      <div class="col-md-4" v-for="n in 3" :key="'skel-top-' + n">
        <div
          class="card custom-card dashboard-main-card dashboard-loading-card border-0 shadow-sm"
          style="height: 160px"
        >
          <div class="card-body">
      ...
      <div class="col-12 mt-4 dashboard-loading-status">
        <div class="dashboard-loader-stage" role="status" aria-live="polite">
          <div class="bouncing-loader-orbit" aria-hidden="true">
            <span
              class="bouncing-loader-shadow bouncing-loader-shadow-1"
            ></span>
            <span
              class="bouncing-loader-shadow bouncing-loader-shadow-2"
            ></span>
            <span
              class="bouncing-loader-shadow bouncing-loader-shadow-3"
            ></span>
    ...
    <template v-else>
      <div class="monitoring-overview animate-show-up">
            ...
            >
              <div class="monitoring-kpi-copy">
                ...
                <strong class="monitoring-kpi-value">
                  ...
                  <small v-if="card.total" class="monitoring-kpi-total"
                    >/{{ formatChartNumber(card.total) }}</small
                  >
                </strong>
                ...
                <div v-if="card.total" class="monitoring-kpi-progress">
                  <i
                    :style="{
                      width: monitoringPercentOf(card.value, card.total) + '%',
                    }"
                  ></i>
                </div>
              </div>
              <div
                ...
                :style="{
                  '--kpi-rate':
                    (card.rate ?? monitoringPercentOf(card.value, card.total)) +
                    '%',
                }"
              >
                <span
        ...
        <div class="monitoring-grid">
          <section
            class="monitoring-panel dashboard-distribution-panel dashboard-distribution-panel-full"
          >
            ...
            <div
              class="dashboard-distribution-scroll dashboard-distribution-scroll--sector"
            >
          ...
          >
            ...
            <div
              class="dashboard-distribution-scroll dashboard-distribution-scroll--subsector"
            >
              <div class="dashboard-distribution-chart-frame">
                <apexchart
          ...
          <section class="monitoring-panel monitoring-panel-compact">
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-shield-star-line"></i> KSE</span
                >
            ...
            <div class="monitoring-legend">
              <span
                v-for="(label, index) in monitoringKseData.labels"
                :key="'monitoring-kse-' + label"
              >
                <i :style="{ background: monitoringKseData.colors[index] }"></i>
                {{ label }}
                {{ formatChartNumber(monitoringKseData.series[index]) }}
              </span>
            </div>
          </section>

          <section class="monitoring-panel monitoring-panel-ikas-summary">
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-bar-chart-box-line"></i> IKAS</span
                >
                <h3>Level Kematangan IKAS</h3>
                <p>Distribusi level hasil asesmen.</p>
              </div>
              <strong>{{
                Number(ikasSummaryData.avgNilaiKematangan || 0).toFixed(2)
              }}</strong>
            </div>
            <apexchart
              height="255"
            ...
            <div class="monitoring-legend">
              <span
                v-for="(item, index) in monitoringIkasItems"
                :key="'monitoring-ikas-lvl-' + item.label"
              >
          ...
          <section class="monitoring-panel monitoring-panel-compact">
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-shield-check-line"></i> CSIRT</span
                >
            ...
            <div class="monitoring-mini-stats">
              <div>
                <strong>{{
                  formatChartNumber(csirtStatus.sudah_membentuk_csirt)
                }}</strong>
                <span>Sudah membentuk</span>
              </div>
              <div>
                <strong>{{
                  formatChartNumber(csirtStatus.belum_membentuk_csirt)
                }}</strong>
          ...
          <section class="monitoring-panel monitoring-panel-ikas-bars">
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-focus-3-line"></i> IKAS</span
                >
              ...
              >
                ...
                <div class="ki-domain-info">
                  ...
                  <div class="ki-domain-score-bar">
                    <div
                      class="ki-domain-fill"
                      :style="{
                        width: Math.min(100, (domain.value / 5) * 100) + '%',
                        background: domain.color,
                      }"
                    ></div>
                  </div>
                </div>
                <span class="ki-domain-val" :style="{ color: domain.color }">{{
          ...
          <section class="monitoring-panel monitoring-panel-ikas-pie">
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-pie-chart-2-line"></i> IKAS</span
                >
            ...
            <div class="ki-domain-pie-panel-body">
              ...
              <div class="monitoring-legend ki-domain-pie-legend">
                <span
                  v-for="domain in monitoringIkasDomainItems"
                  :key="'monitoring-pie-legend-' + domain.key"
                >
          ...
          <section
            class="monitoring-panel monitoring-panel-compact monitoring-panel-status"
          >
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-list-check-3"></i> Status</span
                >
          ...
          <section
            class="monitoring-panel monitoring-panel-compact monitoring-panel-risk"
          >
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-shield-flash-line"></i> Manris</span
                >
            ...
            <div class="monitoring-risk-body">
              <div
                class="monitoring-risk-meter"
                :style="{ '--risk-rate': monitoringRiskSummary.rate + '%' }"
              >
                <span></span>
              </div>
              <div class="monitoring-mini-stats">
                <div>
                  <strong>{{
                    formatChartNumber(monitoringRiskSummary.completed)
                  }}</strong>
                  <span>Selesai</span>
                </div>
                <div>
                  <strong>{{
                    formatChartNumber(monitoringRiskSummary.inProgress)
                  }}</strong>
                  <span>Proses</span>
                </div>
                <div>
                  <strong>{{
                    formatChartNumber(monitoringRiskSummary.notStarted)
                  }}</strong>
                  <span>Belum</span>
                </div>
              </div>
              <button
                class="monitoring-risk-action"
                @click="router.push('/stakeholders')"
              >
          ...
          <section class="monitoring-panel monitoring-panel-top">
            <div class="monitoring-panel-header">
              <div>
                <span class="monitoring-panel-badge"
                  ><i class="ri-layout-grid-line"></i> Top</span
                >
            ...
            <div class="monitoring-top-list">
              ...
              >
                ...
                <small
                  >{{
                    monitoringPercentOf(item.value, totalStakeholders)
                  }}%</small
                >
              </div>
            </div>
      ...
      <template v-if="false">
        ...
        <div
          v-if="filterStore.sektorId"
          class="d-flex align-items-center gap-2 mb-2 mt-1 animate-show-up"
          :style="{ animationDelay: isFirstLoad ? '0.1s' : '0s' }"
        >
          <span
            class="badge bg-primary-transparent text-primary d-inline-flex align-items-center gap-1"
          ...
          <span class="text-muted" style="font-size: 0.75rem"
            >{{ sektorCards.length }} item</span
          >
        </div>
        <div class="row g-3">
          <div
            :class="[
              sektorCards.length === 1
                ? 'col-xl-4 col-md-6'
                : sektorCards.length === 4
                ? 'col-xl-3 col-md-6'
                : sektorCards.length <= 6
                ? 'col-xl-4 col-md-6'
                : 'col-xl-3 col-md-4',
              'animate-show-up',
            ]"
            ...
            :key="'sektor-' + index"
            :style="{
              animationDelay: isFirstLoad ? `${0.1 + index * 0.05}s` : '0s',
            }"
        ...
        <div v-if="operationalCardsReady" class="row g-3 mt-1">
          <div
            class="col-xl-3 col-md-6 animate-show-up"
            v-for="(card, index) in operationalCards"
            :key="'ops-' + index"
            :style="{
              animationDelay: isFirstLoad ? `${0.4 + index * 0.1}s` : '0s',
            }"
        ...
        <div class="row g-3 mb-4">
          <div
            class="col-xl-4 animate-show-up"
            :style="{ animationDelay: isFirstLoad ? '1.0s' : '0s' }"
          >
            <InsightCard
              :loading="!dashboardDetailsReady"
              @drill-down="handleDrillDown"
            />
          </div>
          <div
            class="col-xl-4 animate-show-up"
            :style="{ animationDelay: isFirstLoad ? '1.2s' : '0s' }"
          >
            <ActionCenter :loading="!dashboardDetailsReady" />
          </div>
          <div
            class="col-xl-4 animate-show-up"
            :style="{ animationDelay: isFirstLoad ? '1.4s' : '0s' }"
          >
        ...
        <div
          v-if="filterStore.isLoading && !filterStore.summaryData"
          class="row g-3 mb-4"
        >
          <div
          ...
          >
            <div class="card custom-card">
              <div class="card-body p-3">
                <div
                  class="skeleton-icon shimmer"
                  style="width: 40px; height: 40px; border-radius: 10px"
                ></div>
                <div
                  class="skeleton-text shimmer mt-3"
                  style="width: 50%; height: 12px"
                ></div>
                <div
                  class="skeleton-number shimmer mt-2"
                  style="width: 35%; height: 22px"
                ></div>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <button
            ...
            @click="filterStore.fetchDashboardData"
          >
        ...
        <template v-else-if="filterStore.summaryData">
          <div class="row g-3 mb-4">
            <div class="col-xl-12">
              ...
              <div v-if="false" class="row g-3 mb-4 align-items-stretch">
                ...
                <div
                  ...
                  :style="{ animationDelay: isFirstLoad ? '1.3s' : '0s' }"
                >
                  <!-- 1. LIST STAKEHOLDER (Jika 1 Sub Sektor Dipilih) -->
                  <div
                    v-if="
                      filterStore.subSektorId &&
                      filterStore.subSektorId !== 'ALL'
                    "
                    class="card custom-card mb-0 h-100"
                  >
                    <div class="card-header d-flex flex-column gap-3 py-3">
                      ...
                      >
                        <div class="d-flex align-items-center gap-2">
                          ...
                          <div>
                            <h6 class="mb-0 fw-bold">
                              {{
                                activeSubSektorSummary?.nama_sektor ||
                                "Data Stakeholder Aktif"
                              }}
                            </h6>
                            <small class="text-muted"
                              >{{ baseStakeholders.length }} stakeholder di sub
                              sektor ini</small
                            >
                          </div>
                        </div>
                        ...
                        <div
                          v-if="activeSubSektorSummary"
                          class="d-flex gap-3 align-items-center"
                        >
                          <div class="text-center">
                            <span class="text-muted fs-11 d-block fw-medium"
                              >TOTAL</span
                            >
                            <span
                              class="badge bg-primary-transparent fw-bold"
                              >{{ activeSubSektorSummary.total }}</span
                            >
                          </div>
                          <div class="text-center">
                            <span class="text-muted fs-11 d-block fw-medium"
                            ...
                            >
                            <span
                              v-if="activeSubSektorSummary.countYear > 0"
                              class="badge bg-primary-transparent fw-bold"
                              >{{ activeSubSektorSummary.countYear }}</span
                            >
                          ...
                          <div class="text-center">
                            ...
                            >
                            <span
                              v-if="activeSubSektorSummary.countQuarter > 0"
                              class="badge bg-primary-transparent fw-bold"
                              >{{ activeSubSektorSummary.countQuarter }}</span
                            >
                            <span v-else class="text-muted small">0</span>
                          </div>
                          <div class="text-center">
                            ...
                            >
                            <span
                              v-if="activeSubSektorSummary.this_month > 0"
                              class="badge bg-primary-transparent fw-bold"
                              >+{{ activeSubSektorSummary.this_month }}</span
                            >
                            <span v-else class="text-muted small">0</span>
                          </div>
                        </div>
                      ...
                      <div
                        v-if="activeSubSektorSummary"
                        class="d-flex w-100 gap-4 mt-2 mb-1 animate-show-up"
                        :style="{ animationDelay: isFirstLoad ? '1.4s' : '0s' }"
                      >
                        <div class="flex-grow-1">
                          ...
                          >
                            ...
                            <span class="fs-11 fw-bold text-primary">{{
                              activeSubSektorSummary.csirtPercent
                            }}</span>
                          </div>
                          <div class="progress" style="height: 4px">
                            <div
                              ...
                              role="progressbar"
                              :style="{
                                width: activeSubSektorSummary.csirtPercent,
                              }"
                            ></div>
                          </div>
                        </div>
                        <div class="flex-grow-1">
                          ...
                          >
                            ...
                            >
                            <span class="fs-11 fw-bold text-primary">{{
                              activeSubSektorSummary.ikasPercent
                            }}</span>
                          </div>
                          <div class="progress" style="height: 4px">
                            <div
                              ...
                              role="progressbar"
                              :style="{
                                width: activeSubSektorSummary.ikasPercent,
                              }"
                            ></div>
                          </div>
                        </div>
                        <div class="flex-grow-1">
                          ...
                          >
                            ...
                            >
                            <span class="fs-11 fw-bold text-primary">{{
                              activeSubSektorSummary.lengkapPercent
                            }}</span>
                          </div>
                          <div class="progress" style="height: 4px">
                            <div
                              ...
                              role="progressbar"
                              :style="{
                                width: activeSubSektorSummary.lengkapPercent,
                              }"
                            ></div>
                          </div>
                        </div>
                    ...
                    <div
                      class="card-body p-0 animate-show-up"
                      :style="{ animationDelay: isFirstLoad ? '1.5s' : '0s' }"
                    >
                      <div
                      ...
                      >
                        <table class="table table-hover mb-0 fs-sektor-table">
                          ...
                          <tbody>
                            <tr
                              v-for="(s, idx) in baseStakeholders"
                              :key="s.id"
                            >
                              <td class="text-muted">{{ idx + 1 }}</td>
                              <td>
                                <span class="fw-medium text-primary">{{
                                  s.nama_perusahaan || s.nama || "Tanpa Nama"
                                }}</span>
                              </td>
                              <td class="text-center">
                                <div
                                  v-if="
                                    csirtStore.csirts.some(
                                      (c) =>
                                        String(c.id_perusahaan) ===
                                          String(s.id) ||
                                        String(c.perusahaan?.id) ===
                                          String(s.id),
                                    )
                                  "
                                >
                                  <i
                                    class="ri-checkbox-circle-fill text-primary fs-18"
                                    v-if="csirtStore.hasCompleteCsirt(s.id)"
                                    title="CSIRT Lengkap"
                                  ></i>
                                  <i
                                    class="ri-checkbox-circle-line text-muted fs-18"
                                    v-else
                                    title="CSIRT Belum Lengkap (Hanya Registrasi)"
                                  ></i>
                                </div>
                  ...
                  <div
                    v-else-if="apiSektorCounts.length"
                    class="card custom-card mb-0 h-100"
                  >
                    <div
                    ...
                    >
                      <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                          <h6 class="mb-0 fw-bold">
                            Distribusi Stakeholder per
                            {{ filterStore.sektorId ? "Sub Sektor" : "Sektor" }}
                          </h6>
                          <small class="text-muted"
                            >{{ apiSektorCounts.length }}
                            {{
                              filterStore.sektorId ? "sub sektor" : "sektor"
                            }}</small
                          >
                    ...
                    <div class="card-body p-0">
                      <div class="table-responsive">
                        <table class="table table-hover mb-0 fs-sektor-table">
                          ...
                          <tbody>
                            <tr
                              v-for="(sektor, idx) in apiSektorCounts"
                              :key="sektor.id"
                              @click="
                                handleDrillDown({ type: sektor.nama_sektor })
                              "
                            ...
                            >
                              ...
                              <td class="text-center">
                                <span
                                  v-if="sektor.countYear > 0"
                                  class="badge bg-info-transparent fw-bold"
                                  >{{ sektor.countYear }}</span
                                >
                                <span v-else class="text-muted small">0</span>
                              </td>
                              <td class="text-center">
                                <span
                                  v-if="sektor.countQuarter > 0"
                                  class="badge bg-info-transparent fw-bold"
                                  >{{ sektor.countQuarter }}</span
                                >
                                <span v-else class="text-muted small">0</span>
                              </td>
                              <td class="text-center">
                                <span
                                  v-if="sektor.this_month > 0"
                                  class="badge bg-success-transparent fw-bold"
                                >
                                  +{{ sektor.this_month }}
                                </span>
                                <span v-else class="text-muted small">0</span>
              ...
              <div v-if="lowerSectionsReady" class="kse-ikas-analytics mb-4">
                ...
                <div
                  class="ki-command-center mb-3 animate-show-up"
                  :style="{ animationDelay: isFirstLoad ? '1.5s' : '0s' }"
                >
                  <div class="ki-main-header">
                    <div class="ki-header-left">
                      ...
                      <div class="ki-segmented-control">
                        ...
                        <button
                          ...
                          @click="summaryMode = 'IKAS'"
                        >
                          IKAS
                        </button>
                        <button
                          class="ki-seg-btn"
                          :class="{ active: summaryMode === 'CSIRT' }"
                          @click="summaryMode = 'CSIRT'"
                        >
                          CSIRT
                        </button>
                        <button
                          class="ki-seg-btn"
                          :class="{ active: summaryMode === 'MANRIS' }"
                          @click="summaryMode = 'MANRIS'"
                        >
                          Manris
                        </button>
                      </div>
                    ...
                    <div class="ki-header-right">
                      <!-- Category Filters -->
                      <div
                        class="ki-inline-filters"
                        v-if="summaryMode === 'KSE'"
                      >
                        <button
                          v-for="cat in ['Strategis', 'Tinggi', 'Rendah']"
                          :key="cat"
                          class="ki-filter-pill"
                          :class="
                            filterStore.kategoriSe === cat ? 'active' : ''
                          "
                          @click="
                            filterStore.setKategoriSe(
                              cat === filterStore.kategoriSe ? '' : cat,
                            )
                          "
                        >
                          {{ cat }}
                      ...
                      <div class="ki-view-toggles">
                        <button
                          class="ki-view-btn"
                          :class="{ active: kseIkasViewMode === 'overview' }"
                          @click="kseIkasViewMode = 'overview'"
                        ...
                        <button
                          class="ki-view-btn"
                          :class="{ active: kseIkasViewMode === 'table' }"
                          @click="kseIkasViewMode = 'table'"
                          title="Metric Cards"
                        >
                  ...
                  <div
                    v-if="kseIkasViewMode === 'overview'"
                    class="ki-hero-banner"
                  >
                    ...
                    <div class="ki-hero-inner">
                      ...
                      <div class="ki-ring-wrap">
                        <svg viewBox="0 0 120 120" class="ki-ring-svg">
                          ...
                          <circle
                            ...
                            :stroke-dashoffset="
                              326.73 - (326.73 * activeProgressRate) / 100
                            "
                        ...
                        <div class="ki-ring-label">
                          <span class="ki-ring-value"
                            >{{ activeProgressRate }}%</span
                          >
                      ...
                      <div class="ki-hero-stats">
                        <h5 class="ki-hero-title">
                          Progress {{ summaryMode }}
                        </h5>
                        <p class="ki-hero-desc">
                          Status kelengkapan data periode aktif
                        </p>
                      ...
                      <button
                        class="ki-hero-cta"
                        @click="router.push(activeDetailRoute)"
                      >
                ...
                <div
                  v-if="kseIkasViewMode === 'table'"
                  class="row g-3 animate-show-up"
                  :style="{ animationDelay: '0.1s' }"
                >
                  <div
                    v-for="(item, idx) in fullSummaryItems"
                    :key="summaryMode + '-fs-' + idx"
                    :class="
                      summaryMode === 'KSE'
                        ? idx < 2
                          ? 'col-xl-6'
                  ...
                  >
                    <div
                      class="ki-metric-card"
                      :class="{
                        'ki-metric-active':
                          filterStore.kategoriSe === item.category &&
                          item.category,
                        'ki-metric-muted': item.isMuted,
                      }"
                      @click="!item.isMuted && handleKseCardClick(item)"
                      :style="{ '--ki-accent': item.color }"
                    >
                      <div
                        class="ki-metric-accent"
                        ...
                        <div class="ki-metric-top">
                          ...
                          <span
                            v-if="
                              ...
                              item.category
                            "
                            class="ki-metric-active-badge"
                            >Aktif</span
                          >
                        </div>
                        <div class="ki-metric-value">
                ...
                <div v-else class="ki-charts-section">
                  <div class="row g-3 mt-3">
                    <!-- Main Chart Column -->
                    <div
                      class="col-xl-8 animate-show-up"
                      ...
                      <div class="ki-chart-card h-100">
                        <div class="ki-chart-header">
                          <div class="ki-chart-header-left">
                            ...
                            <div>
                              <div class="ki-chart-title">
                                Visualisasi {{ summaryMode }}
                              </div>
                              <div class="ki-chart-sub">
                                Distribusi dan status data periode aktif
                              </div>
                            </div>
                          </div>
                              ...
                              <div class="ki-visual-strip">
                                ...
                                >
                                  <div class="ki-visual-pill-top">
                                    ...
                                    ></span>
                                    <span class="ki-visual-label">{{
                                      item.label
                                    }}</span>
                                  </div>
                                  <div class="ki-visual-value">
                        ...
                        <div class="ki-breakdown">
                          ...
                          <div class="ki-bp-items" v-if="summaryMode === 'KSE'">
                            <div
                              v-for="(cat, ci) in [
                                {
                                  label: 'Strategis',
                                  value: kseData.strategis,
                                  color: '#e6533c',
                                  icon: 'ri-shield-star-fill',
                                },
                                {
                                  label: 'Tinggi',
                                  value: kseData.tinggi,
                                  color: '#f5b849',
                                  icon: 'ri-arrow-up-circle-fill',
                                },
                                {
                                  label: 'Rendah',
                                  value: kseData.rendah,
                                  color: '#26bf94',
                            ...
                            >
                              <div class="ki-bp-left">
                                ...
                                <div>
                                  ...
                                  <div class="ki-bp-count">
                                    {{ cat.value }} <span>SE</span>
                                  </div>
                                </div>
                              </div>
                              <div class="ki-bp-bar-wrap">
                                <div class="ki-bp-bar">
                                  <div
                                    class="ki-bp-bar-fill"
                                    :style="{
                                      width:
                                        (kseData.total_se
                                          ? Math.max(
                                              3,
                                              (cat.value / kseData.total_se) *
                                                100,
                                            )
                                          : 0) + '%',
                                ...
                                <span
                                  ...
                                  >{{
                                    kseData.total_se
                                      ? Math.round(
                                          (cat.value / kseData.total_se) * 100,
                                        )
                                      : 0
                                  }}%</span
                                >
                          ...
                          >
                            <div
                              ...
                              :key="lvl.key"
                              class="ki-bp-row"
                              :style="{ animationDelay: li * 0.06 + 's' }"
                            >
                              <div class="ki-bp-left">
                                <div
                                  class="ki-bp-dot"
                                ...
                                <div>
                                  ...
                                  <div class="ki-bp-count">
                                    {{
                                      ikasSummaryData.levels[lvl.key]?.count ??
                                      0
                                    }}
                                  </div>
                                </div>
                              ...
                              <div class="ki-bp-bar-wrap">
                                <div class="ki-bp-bar">
                                  <div
                                    ...
                                    :style="{
                                      width:
                                        (ikasSummaryData.total
                                          ? Math.max(
                                              3,
                                              (ikasSummaryData.levels[lvl.key]
                                                ?.count /
                                                ikasSummaryData.total) *
                                                100,
                                            )
                          ...
                          <div
                            class="ki-bp-items"
                            v-else-if="summaryMode === 'CSIRT'"
                          >
                            <div
                              v-for="(item, ci) in [
                                {
                                  label: 'Lengkap',
                                  value: csirtData.lengkap,
                                  total: csirtData.total_csirt,
                                  color: '#26bf94',
                                  icon: 'ri-checkbox-circle-fill',
                                },
                                {
                                  label: 'Belum Lengkap',
                                  value: csirtData.belum_lengkap,
                                  total: csirtData.total_csirt,
                                  color: '#f5b849',
                            ...
                            >
                              <div class="ki-bp-left">
                                ...
                                <div>
                                  <div class="ki-bp-label">
                                    {{ item.label }}
                                  </div>
                                  <div class="ki-bp-count">
                                    {{ item.value }} <span>CSIRT</span>
                                  </div>
                                </div>
                              </div>
                              <div class="ki-bp-bar-wrap">
                          ...
                          <div class="ki-bp-items" v-else>
                            <div
                              v-for="(item, ci) in [
                                {
                                  label: 'Sudah Diisi',
                                  value: manrisStatus.sudah_mengisi_manris,
                                  total: manrisStatus.total_perusahaan,
                                  color: '#26bf94',
                                  icon: 'ri-checkbox-circle-fill',
                                },
                                {
                                  label: 'Belum Diisi',
                                  value: manrisStatus.belum_mengisi_manris,
                                  total: manrisStatus.total_perusahaan,
                                  color: '#e6533c',
                                ...
                                {
                                  label: 'Responden',
                                  value: resikoStore.adminRespondents.length,
                                  total: Math.max(
                                    resikoStore.adminRespondents.length,
                                    1,
                                  ),
                                  color: '#0ea5e9',
                        ...
                        <div class="ki-insight-box">
                                    ...
                                    <div
                                      ...
                                      :style="{
                                        width:
                                          Math.min(
                                            100,
                                            (Number(ikasSummaryData[dom.key]) /
                                              5) *
                                              100,
                                          ) + '%',
                          ...
                          <template v-else-if="summaryMode === 'KSE'">
                            <div class="ki-insight-content">
                              ...
                              <h6 class="ki-insight-title">
                                Analisa Strategis
                              </h6>
                              <p class="ki-insight-text">
                                Terdapat
                                <strong>{{ kseData.strategis }}</strong> SE
                                Strategis dari total
                                <strong>{{ kseData.total_se }}</strong> Sistem
                                Elektronik. Fokus pengawasan pada SE Strategis
                                untuk meminimalisir risiko operasional.
                              </p>
                              <div class="ki-insight-tip">
                                <i class="ri-information-line"></i>
                                <span
                                  >SE Strategis membutuhkan audit keamanan
                                  berkala</span
                                >
                              </div>
                            </div>
                          ...
                          <template v-else-if="summaryMode === 'CSIRT'">
                            <div class="ki-insight-content">
                              ...
                              <p class="ki-insight-text">
                                Terdapat
                                <strong>{{ csirtData.lengkap }}</strong> CSIRT
                                lengkap dari total
                                <strong>{{ csirtData.total_csirt }}</strong>
                                CSIRT pada periode aktif.
                              </p>
                              <div class="ki-insight-tip">
                                <i class="ri-information-line"></i>
                                <span
                                  >CSIRT lengkap minimal memiliki data SDM dan
                                  Sistem Elektronik</span
                                >
                              </div>
                            </div>
                          </template>
                          <template v-else>
                            <div class="ki-insight-content">
                              ...
                              <p class="ki-insight-text">
                                Terdapat
                                <strong>{{
                                  manrisStatus.sudah_mengisi_manris
                                }}</strong>
                                perusahaan yang sudah memiliki responden survey
                                risiko dari total
                                <strong>{{
                                  manrisStatus.total_perusahaan
                                }}</strong>
                                stakeholder.
                              </p>
                              <div class="ki-insight-tip">
                                <i class="ri-information-line"></i>
                                <span
                                  >Data diambil dari endpoint responden survey
                                  risiko admin</span
                                >
                              </div>
                            </div>
                          ...
                          <button
                            ...
                            @click="router.push(activeDetailRoute)"
                          >
                            <i class="ri-external-link-line"></i> Lihat Detail
                            {{ summaryMode }}
                          </button>
                        </div>
                      </div>
            ...
            <div v-if="!lowerSectionsReady" class="row g-3 mb-4">
              <div class="col-xl-8">
                <div
                  class="card custom-card border-0 shadow-sm"
                  ...
                  <div class="card-body placeholder-glow">
                    ...
                    <div class="row g-3">
                      <div
                        class="col-md-4"
                        v-for="n in 3"
                        :key="'analysis-skeleton-' + n"
                      >
                        <span
                          class="placeholder col-12 mb-2"
                          style="height: 70px; border-radius: 12px"
                        ></span>
                        <span
                ...
                >
                  <div class="card-body placeholder-glow">
                    ...
                    >
                      ...
                      <div class="flex-grow-1">
                        <span
                          class="placeholder col-8 d-block mb-2"
                          style="height: 12px; border-radius: 6px"
                        ></span>
                        <span
                          class="placeholder col-5 d-block"
...
.active-filter-card {
  background: linear-gradient(
    135deg,
    rgba(var(--dw-primary-rgb), 0.05) 0%,
    #fff 100%
  ) !important;
...
.dashboard-loading-card {
  ...
  border-color: var(
    --dashboard-skeleton-card-border,
    rgba(15, 23, 42, 0.06)
  ) !important;
}
...
.dashboard-loader-stage {
  ...
  background: linear-gradient(
      180deg,
      var(--dashboard-loader-surface, rgba(255, 255, 255, 0.88)),
      transparent
    ),
    radial-gradient(circle at 50% 0%, rgba(56, 189, 248, 0.14), transparent 58%);
  box-shadow: var(
    --dashboard-loader-shadow,
    0 20px 46px rgba(37, 99, 235, 0.12)
  );
...
.bouncing-loader-ball {
  ...
  height: 18px;
  border-radius: 999px;
  box-shadow: inset -4px -5px 8px rgba(15, 23, 42, 0.18),
    0 12px 22px rgba(37, 99, 235, 0.2);
  transform-origin: center bottom;
  animation: dashboardBallBounce 0.88s cubic-bezier(0.34, 1.56, 0.64, 1)
    infinite;
...
.bouncing-loader-ball-1 {
  ...
  background: radial-gradient(
    circle at 35% 28%,
    #bfdbfe 0 14%,
    var(--dashboard-ball-one, #2563eb) 56%,
    #1d4ed8 100%
  );
}
...
.bouncing-loader-ball-2 {
  ...
  background: radial-gradient(
    circle at 35% 28%,
    #cffafe 0 14%,
    var(--dashboard-ball-two, #0891b2) 58%,
    #0e7490 100%
  );
...
.bouncing-loader-ball-3 {
  ...
  background: radial-gradient(
    circle at 35% 28%,
    #bbf7d0 0 14%,
    var(--dashboard-ball-three, #059669) 58%,
    #047857 100%
  );
...
.bouncing-loader-shadow {
  ...
  animation: dashboardBallShadow 0.88s cubic-bezier(0.34, 1.56, 0.64, 1)
    infinite;
}

.bouncing-loader-shadow-1 {
  left: 26px;
}
.bouncing-loader-shadow-2 {
  left: 53px;
  animation-delay: 0.12s;
}
.bouncing-loader-shadow-3 {
  left: 80px;
  animation-delay: 0.24s;
}
...
@keyframes dashboardBallBounce {
  0%,
  100% {
    transform: translateY(24px) scale(1.08, 0.84);
  }
  15% {
    transform: translateY(24px) scale(1.12, 0.78);
  }
  50% {
...
.fs-filter-pill-alt.active {
  background: #2563eb;
  color: #fff;
  border-color: #2563eb;
  box-shadow: 0 4px 12px rgba(37, 99, 235, 0.2);
}
.active-filter-card-alt {
...
.ki-main-header::before {
  ...
  inset: 0;
  width: auto;
  background: linear-gradient(90deg, rgba(255, 255, 255, 0.14), transparent 34%),
    radial-gradient(
      circle at 88% 50%,
...
.ki-segmented-control {
  display: flex;
  background: rgba(10, 34, 111, 0.28);
  padding: 4px;
  border-radius: 11px;
  gap: 2px;
  box-shadow: inset 0 0 0 1px rgba(255, 255, 255, 0.12);
...
.ki-filter-pill:hover {
  background: rgba(255, 255, 255, 0.16);
  border-color: rgba(255, 255, 255, 0.55);
  color: #fff;
}
.ki-filter-pill.active {
  background: #fff;
  color: var(--ki-brand);
  border-color: transparent;
...
.ki-metric-card:hover {
  transform: translateY(-3px);
  box-shadow: 0 18px 38px rgba(28, 46, 86, 0.13);
}
.ki-metric-accent {
  height: 3px;
  width: 100%;
...
.ki-metric-active {
  ...
  transform: translateY(-3px);
  box-shadow: 0 15px 32px rgba(37, 99, 235, 0.15) !important;
}
.ki-metric-muted {
  opacity: 0.4;
  cursor: not-allowed !important;
...
.ki-hero-glow {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(90deg, rgba(37, 99, 235, 0.045), transparent 34%),
...
.ki-chart-header-compact .ki-chart-icon-wrap {
  width: 38px;
  height: 38px;
  border-radius: 11px;
}
.ki-chart-icon-wrap-soft {
...
.ki-ct-btn.active {
  background: #fff;
  color: var(--ki-brand);
  box-shadow: 0 2px 8px rgba(28, 46, 86, 0.08);
}
.ki-chart-body {
...
:global(body[data-theme-mode="dark"])
  #dashboard-capture
  .dashboard-loading-card,
...
.monitoring-readiness-ring {
  ...
  display: grid;
  place-items: center;
  background: radial-gradient(circle at center, #ffffff 0 56%, transparent 57%),
...
.monitoring-kpi {
  ...
  background: linear-gradient(
      180deg,
      color-mix(in srgb, var(--monitoring-accent) 10%, #ffffff) 0%,
      #ffffff 62%
    ),
    #ffffff;
  border-color: color-mix(
    in srgb,
    var(--monitoring-accent) 28%,
...
.monitoring-kpi:hover {
  background: linear-gradient(
      180deg,
      color-mix(in srgb, var(--monitoring-accent) 15%, #ffffff) 0%,
      #ffffff 64%
    ),
    #ffffff;
  border-color: color-mix(
    in srgb,
    var(--monitoring-accent) 58%,
    rgba(226, 232, 240, 0.96)
  );
...
.monitoring-kpi-donut {
  ...
  display: grid;
  place-items: center;
  background: radial-gradient(circle at center, #ffffff 0 55%, transparent 56%),
    conic-gradient(
      var(--monitoring-accent) var(--kpi-rate),
      color-mix(in srgb, var(--monitoring-accent) 14%, #e2e8f0) 0
    );
...
.monitoring-kpi {
  min-height: 96px;
  padding: 0.85rem 0.92rem 0.8rem;
  gap: 0.7rem;
}
...
.dashboard-capture.is-dark .monitoring-readiness-ring {
  background: radial-gradient(circle at center, #111827 0 56%, transparent 57%),
